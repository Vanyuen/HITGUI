/**
 * 通过API重新创建任务1 (101期: 25042-25142)
 * 任务配置与原始hwc-pos-20251213-ykt相同
 */

const http = require('http');

const taskConfig = {
    task_name: "重新运行任务1_验证修复_25042-25142",
    period_range: {
        type: "custom",
        value: {
            start: "25042",
...
console.log('创建任务1 (101期: 25042-25142)...');
console.log('注意: 此任务有101期，可能需要几分钟完成\n');
    ...
    res.on('end', () => {
        try {
            ...
            if (result.success) {
                ...
                console.log('任务已开始执行，请等待完成...');
                console.log('\n运行以下命令监控任务进度:');
                console.log(`  curl -s "http://localhost:3003/api/dlt/hwc-positive-tasks/${result.data.task_id}"`);
            } else {
