...
async function fixHWCTableRegeneration() {
    try {
        ...
        console.log('🎉 重建结果:');
        console.log(`   - 总处理记录数: ${bulkWriteResult.upsertedCount + bulkWriteResult.modifiedCount}`);
        console.log(`   - 新插入记录数: ${bulkWriteResult.upsertedCount}`);
        console.log(`   - 更新现有记录数: ${bulkWriteResult.modifiedCount}`);
